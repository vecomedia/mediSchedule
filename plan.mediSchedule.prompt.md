...
### Phase 2B: Backend Transition (Prisma + PostgreSQL)

16. Add Prisma + PostgreSQL setup:
  - `prisma/schema.prisma`
  - `src/lib/prisma.ts`
  - scripts in `package.json` for generate/migrate/seed
17. Keep existing object contracts while moving to Zod-first domain schemas:
  - source of truth in `src/lib/schemas/domain.ts`
  - infer TS types in `src/lib/types.ts`
18. Introduce transaction-safe appointment services:
  - create/reschedule/cancel/update in `src/lib/server/appointment-service.ts`
  - deterministic overlap checks for the same doctor
  - idempotency support for retried create requests
19. Add API endpoints for booking/editing:
  - `src/app/api/appointments/route.ts` (GET/POST)
  - `src/app/api/appointments/[appointmentId]/route.ts` (PATCH)
20. Next wiring step (separate): connect staff and patient dialogs/forms to these APIs and replace in-memory reads from `src/lib/data.ts`.
