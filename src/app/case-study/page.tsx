...
export default function CaseStudyPage() {
  ...
  auth.ts             ← NextAuth config, credential provider`}</pre>
						...
						<Decision
							...
							reason="The appointment-form component uses RHF with a Zod resolver. This keeps validation logic centralized in the schema, makes error messages declarative, and avoids manual onChange handlers for every field."
						/>
						<Decision
							...
							<li>
								...
								everywhere — <code>session.user.role</code> is typed throughout.
									...
									<Subsection title="What I would do differently">
										<ul className="space-y-3 text-slate-700 leading-7 list-disc list-inside">
							<li>
								Replace the in-memory data store with a proper database (SQLite via Drizzle would
								be the natural next step — zero infra, same DX).
							</li>
							<li>
